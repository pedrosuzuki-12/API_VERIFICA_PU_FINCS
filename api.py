...
def verificar_pu():
    # --- INÍCIO DA VERIFICAÇÃO DE SEGURANÇA (BEARER TOKEN) ---
    auth_header = request.headers.get("Authorization")
    
    if not auth_header or not auth_header.startswith("Bearer "):
        return jsonify({"erro": "Acesso negado. Token ausente ou formato inválido (use Bearer)."}), 401
        
    # Pega apenas a parte do token (depois da palavra 'Bearer ')
    token_recebido = auth_header.split(" ")[1]
